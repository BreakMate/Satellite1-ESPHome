...
async def to_code(config):
    ...
    # Both helpers only exist on ESPHome >= 2026.9; older versions still include "json" by
    # default and don't need (or have) this, so skip it there rather than crashing codegen.
    include_builtin = getattr(esp32, "include_builtin_idf_component", None)
    idf_version = getattr(esp32, "idf_version", None)
    if include_builtin is not None and idf_version is not None:
        if idf_version().major >= 6:
            esp32.add_idf_component(name="espressif/cjson", ref="^1.7.19")
        else:
            include_builtin("json")
